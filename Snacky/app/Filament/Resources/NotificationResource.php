...
class NotificationResource extends Resource
{
    ...
    public static function getNavigationBadge(): ?string
    {
        ...
        return $notCount !== 0 ? (string) $notCount : null;
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            //->recordUrl(fn (Notification $record) => SnackResource::getUrl('view', [$record->snack_id]))
            ->columns([
                Panel::make([
                    Grid::make(2)
                        ->schema([
                            Stack::make([
                                TextColumn::make('status')
                                    ->state(fn (Notification $record) => match ($record->status) {
                                        'SEEN' => 'viewed',
                                        'NOT_SEEN' => 'new',
                                        default => 'undefinded'
                                    })
                                ...
                                TextColumn::make('snack.title_ru')
                                    ->limit(50)
                                    ->url(fn (Notification $record) => SnackResource::getUrl('view', [$record->snack_id])),
                            ])->space(2),
                            Stack::make([
                    ...
                    ->extraAttributes(fn (Notification $record) => match ($record->status) {
                        'NOT_SEEN' => [
                        ...
                        ->schema([
                            ...
                            Stack::make([
                                ...
                                SelectColumn::make('snack.status')
                                    ->hidden(! HelperFunctions::isUser(Auth::user())->isManager())
                                    ->options(function (Notification $record) {
